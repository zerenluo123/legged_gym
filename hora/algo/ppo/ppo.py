...
class PPO(object):
    def __init__(self, env, output_dif, train_config):
        ...
        # ---- Optim ----
        self.algo_cfg = self.train_cfg['PPO']
        self.last_lr = float(self.algo_cfg['learning_rate'])
        self.weight_decay = self.algo_cfg.get('weight_decay', 0.0)
        self.optimizer = torch.optim.Adam(self.model.parameters(), self.last_lr, weight_decay=self.weight_decay)
        # ---- PPO Train Param ----
        self.e_clip = self.algo_cfg['e_clip']
        self.clip_value = self.algo_cfg['clip_value']
        self.entropy_coeff = self.algo_cfg['entropy_coeff']
        self.critic_coef = self.algo_cfg['critic_coef']
        self.bounds_loss_coef = self.algo_cfg['bounds_loss_coef']
        self.gamma = self.algo_cfg['gamma']
        self.tau = self.algo_cfg['tau']
        self.truncate_grads = self.algo_cfg['truncate_grads']
        self.grad_norm = self.algo_cfg['grad_norm']
        self.value_bootstrap = self.algo_cfg['value_bootstrap']
        self.normalize_advantage = self.algo_cfg['normalize_advantage']
        self.normalize_input = self.algo_cfg['normalize_input']
        self.normalize_value = self.algo_cfg['normalize_value']
        # ---- PPO Collect Param ----
        self.horizon_length = self.algo_cfg['horizon_length']
        self.batch_size = self.horizon_length * self.num_actors
        self.minibatch_size = self.algo_cfg['minibatch_size']
        self.mini_epochs_num = self.algo_cfg['mini_epochs']
        assert self.batch_size % self.minibatch_size == 0 or train_config['test']
        # ---- scheduler ----
        self.kl_threshold = self.algo_cfg['kl_threshold']
        self.scheduler = AdaptiveScheduler(self.kl_threshold)
        # ---- Snapshot
        self.save_freq = self.algo_cfg['save_frequency']
        self.save_best_after = self.algo_cfg['save_best_after']
        # ---- Tensorboard Logger ----
        self.extra_info = {}
        ...
        self.agent_steps = 0
        self.max_agent_steps = self.algo_cfg['max_agent_steps']
    ...
    def train_epoch(self):
        ...
        for _ in range(0, self.mini_epochs_num):
            ...
            for i in range(len(self.storage)):
                ...
                loss = a_loss + 0.5 * c_loss * self.critic_coef - entropy * self.entropy_coeff + b_loss * self.bounds_loss_coef
