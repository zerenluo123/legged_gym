...
class VecTask(Env):
    ...
    def step(self, actions: torch.Tensor) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor, Dict[str, Any]]:
        ...
        Returns:
            ...
            Observations are dict of observations (currently only one member called 'obs')
        """
        action_tensor = torch.clip(actions, -self.clip_actions, self.clip_actions).to(self.device)
        # apply actions
        ...
        self.obs_dict['obs'] = torch.clip(self.obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device)
